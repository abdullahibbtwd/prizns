...
import { AUTHOR_SLUG_ALIASES } from './author-slug-aliases';
...
export function authorAliasRedirectLines(): string[] {
  const lines: string[] = [];
  for (const [from, to] of Object.entries(AUTHOR_SLUG_ALIASES)) {
    lines.push(formatMapLine(`/authors/${from}`, `/authors/${to}`));
    lines.push(formatMapLine(`/authors/${from}/`, `/authors/${to}`));
    lines.push(formatMapLine(`/en/authors/${from}`, `/en/authors/${to}`));
    lines.push(formatMapLine(`/en/authors/${from}/`, `/en/authors/${to}`));
  }
  return lines;
}
